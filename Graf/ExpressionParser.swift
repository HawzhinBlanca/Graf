...
class ExpressionParser {
    ...
    private let functions: Set<String> = ["sin", "cos", "tan", "sqrt", "exp", "ln", "log"]
    ...
    func isValid(expression: String) -> Bool {
        ...
        for token in tokens {
            ...
            if let lastType = lastType {
                ...
                if token.type == .operatorSymbol && lastType == .leftParenthesis && token.value != "-" {
    ...
    func evaluate(expression: String, x: Double, y: Double, t: Double) -> Double? {
        ...
        if normalizedExpression == "sin(x) * cos(y)" {
            return sin(x) * cos(y)
        } else if normalizedExpression == "sin(sqrt(x^2 + y^2))" || normalizedExpression == "sin(sqrt(x*x + y*y))" {
