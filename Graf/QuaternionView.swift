...
struct QuaternionView: View {
        ...
        VStack {
            ...
            TabView {
                ...
                VStack(spacing: 12) {
                    ...
                    Text("Rotation Axis: [\(getRotationAxis().x, specifier: "%.2f"), \(getRotationAxis().y, specifier: "%.2f"), \(getRotationAxis().z, specifier: "%.2f")]")
                ...
                VStack(spacing: 15) {
                    ...
                    Link("Learn more from 3Blue1Brown", destination: URL(string: "https://youtu.be/zjMuIxRvygQ")!)
                }
                .padding()
                .tabItem {
    ...
    private func normalizeQuaternion() {
        let length = sqrt(quaternionW*quaternionW + quaternionX*quaternionX + quaternionY*quaternionY + quaternionZ*quaternionZ)
...
class QuaternionRenderer: NSObject, MTKViewDelegate {
    ...
    private func setupRenderPipeline(device: MTLDevice, view: MTKView) {
        do {
            ...
            vertexDescriptor.layouts[0].stride = MemoryLayout<SIMD3<Float>>.stride * 2 + MemoryLayout<SIMD4<Float>>.stride
    ...
    private func drawSingleSphereView(renderEncoder: MTLRenderCommandEncoder) {
        ...
        if showAxisLabels {
            ...
            drawAxes(renderEncoder: renderEncoder, transform: matrix_identity_float4x4, color: SIMD4<Float>(0.5, 0.5, 0.5, 0.5))
    ...
    /// Draw a sphere
    private func drawSphere(renderEncoder: MTLRenderCommandEncoder, transform: matrix_float4x4, color: SIMD4<Float>? = nil) {
        guard let sphereVertexBuffer = sphereVertexBuffer, 
              let sphereIndexBuffer = sphereIndexBuffer else {
            return
        }
    ...
    private func drawAxes(renderEncoder: MTLRenderCommandEncoder, transform: matrix_float4x4, color: SIMD4<Float>? = nil) {
        guard let axisVertexBuffer = axisVertexBuffer,
              let axisIndexBuffer = axisIndexBuffer else {
            return
        }
    ...
    /// Draw a rotation arc
    private func drawArc(renderEncoder: MTLRenderCommandEncoder, transform: matrix_float4x4, angle: Float) {
